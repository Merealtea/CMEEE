...
def get_pos_encoding(embed_size, seq_len):
    ...
    emb = torch.cat([sin_emb, cos_emb],2).flatten(1)
    return emb
...
class Multihead_Attention(nn.Module):
    def __init__(self, hidden_size, num_heads, attn_dropout) -> None:
        ...
        self.u = nn.Parameter(torch.Tensor(self.num_heads, 1, self.per_size))
        self.v = nn.Parameter(torch.Tensor(1, self.num_heads, 1, 1, self.per_size))
...
class Encoder_Layer(nn.Module):
    ...
    def forward(self, inp, rel_pos : torch, mask):
        ...
        inp = x + self.dropout1(inp)

        x = inp
        inp = self.norm2(inp)
        inp = self.dropout2(self.FFN(inp)) + x
        return inp


class Transformer_Encoder(nn.Module):
    def __init__(self, hidden_size, ff_size, num_layers, num_heads, max_len, shared_pos_encoding = True, dropout = defaultdict(int) ):
...
if __name__ == "__main__":
    inp = torch.rand((10, 20, 120))
    pos_s = torch.randint(0, 20, (10,20))
    pos_e = torch.randint(0, 20, (10,20))
    sen_len = torch.randint(0, 10, (10,))
    lat_len = torch.randint(0, 10, (10,))
    dropout = {
